...
## 2026-04-16 — mafia.sii5.ru: убраны сетки «ПАРА 1–5 / 6–10» и дублирующий текст под роликами

- **`mafia_sii5_ru/index.html`**: удалены контейнеры `#pair-grid-1` и `#pair-grid-2` перед «Герои и симбиозы»; отступ заголовка секции скорректирован (`mt-10` / `md:mt-14`).
- **`mafia_sii5_ru/src/main.ts`**: удалены `renderPairGrid`, `setActivePair`, панели под видео (`appendVideoStoryBelow`, `initVideoStoryBelowPanels` и вспомогательная нарезка слайдов); аккордеон «Герои и симбиозы» без изменений по данным.
- **`mafia_sii5_ru/src/styles.css`**: удалены стили `.pair-card` и `.video-story-below*` (огненный текст `.video-story-fire-text` оставлен для регламента).
